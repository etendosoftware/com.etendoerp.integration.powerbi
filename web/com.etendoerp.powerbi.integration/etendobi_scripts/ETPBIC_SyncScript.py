...
try:
    ...
    PATH_VERIFICATION = f'ssh {USER}@{IP} -p {PORT} "test -d \'{PATH}\' && echo 1"'
    ...
    PATH_CREATION = f'ssh {USER}@{IP} -p {PORT} "test -d {PATH}{client}/{org_name} && echo 1"'
    ...
    if PATH_CREATION_OUTPUT != "1":
        CREATE_PATH = f'ssh {USER}@{IP} -p {PORT} "mkdir -p {PATH}{client}/{org_name}"'
    ...
    OPTIONS = f'-av {"--delete" if filesAmt > 0 else ""} -e "ssh -p {PORT}"' # do not delete if no files being uploaded
